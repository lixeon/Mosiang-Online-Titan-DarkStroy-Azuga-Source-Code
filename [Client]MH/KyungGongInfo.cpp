...
//	pFile->GetString(m_Info.KyungGongName);		//경공이름
	...
	m_Info.StartEffect = FindEffectNum(pFile->GetString());		//경공 사용 연출
	m_Info.IngEffect = FindEffectNum(pFile->GetString());			//경공 연출 파일
	m_Info.EndEffect = FindEffectNum(pFile->GetString());			//경공 끝 연출
}