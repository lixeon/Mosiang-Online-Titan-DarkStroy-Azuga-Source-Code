...
BOOL CObjectPropertyList::InsertObject(TCHAR* uszObjName,DWORD flag)
{
	...
	CreateMeshTextItem(txt,*(CMeshFlag*)&flag,uszObjName);
...
//	strtok( szObjName, seps );
	memcpy(szObjName,szFlag,lstrlen(szFlag)-EXTEND_STRING_LEN);
...
//	iLBIndex = SendDlgItemMessage(m_hDlg,IDC_LIST_OBJECTS,LB_FINDSTRING,0,(LPARAM)szFlag);         
	...
	memset(szFlag,0,sizeof(szFlag));
	CreateMeshTextItem(szFlag,*(CMeshFlag*)&flag,szObjName);
	pItem->dwPropertyFlag = *(DWORD*)&flag;
...
//	iLBIndex = SendDlgItemMessage(m_hDlg,IDC_LIST_OBJECTS,LB_FINDSTRING,0,(LPARAM)szFlag);         
	...
	memset(szFlag,0,sizeof(szFlag));
	CreateMeshTextItem(szFlag,*(CMeshFlag*)&pItem->dwPropertyFlag,szObjName);
